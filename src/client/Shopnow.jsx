...
const Shopnow = () => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchProducts())
      ...
      .catch((err) => {
        <ErrorPage error={"Failed to Fetch Products"}/>
        setLoading(false);
      });
  }, [dispatch]);
...
const ProductCard = ({ product, quantity, updateCart, addToCartHandler }) => {
  ...
  return (
    <>
      ...
      <div className="border bg-slate-100 rounded-lg overflow-hidden shadow-md cursor-pointer">
        ...
        <div className="Product-Quantity flex flex-col md:flex-row justify-center items-center p-2 text-center space-y-2 md:space-y-0">
          ...
          />
          <button className="btn text-black rounded-lg mx-2" onClick={() => addToCartHandler(product, quantity)}>
